...
Реализовать fullstack MVP FocusTrack AI: веб-приложение для ведения учебных и профессиональных целей с AI-уточнением, планированием, прогрессом и weekly review.
...
Edge Function `rag-answer` принимает вопрос и список документов. Модель отвечает только по переданному контексту.
...
| `rag-answer`       | ответ по переданным документам |
...
| Нет пользовательской сессии | Не загружать приватные строки, показать demo workspace |
| Ошибка Edge Function        | Показать toast или fallback-сообщение                  |
| OpenRouter timeout/error    | Вернуть безопасный JSON без секретов                   |
| Пустые документы RAG        | Ответить, что данных недостаточно                      |
