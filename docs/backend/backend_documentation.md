...
Защищённые AI/RAG-функции используют общий lightweight rate limit:

- таблица `public.function_invocations` хранит user-scoped события вызовов;
- RLS позволяет пользователю видеть и добавлять только свои invocation-записи;
- helper `supabase/functions/_shared/rate-limit.ts` считает вызовы в текущем окне до дорогих операций;
- лимит по умолчанию: 60 вызовов на функцию за 3600 секунд;
- настройки: `RATE_LIMIT_MAX_REQUESTS`, `RATE_LIMIT_WINDOW_SECONDS`, либо `RATE_LIMIT_<FUNCTION>_MAX`.

При превышении лимита функция возвращает HTTP `429`:
...
Redacted evidence хранится во внешнем teacher-facing пакете верхнего workspace. Публичная документация не содержит tokens, cookies, provider screenshots или локальные абсолютные пути.
