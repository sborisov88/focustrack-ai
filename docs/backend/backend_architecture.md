...
Backend-слой реализован: миграции, RLS policies, grants, Edge Functions, health endpoint, vector RAG и frontend-интеграция находятся в репозитории. Следующие product-итерации относятся к расширению возможностей, а не к базовому подключению backend-а.
