...
FocusTrack AI — AI-планировщик личных и рабочих целей с AI-уточнением, планом задач, прогрессом, еженедельным AI-ревью и controlled-RAG ответами по переданным личным заметкам.
...
7. Controlled-RAG эксперимент по личным заметкам (журнал тренировок, бюджет, план IELTS) без embedding index.
...
5. перейти на `/knowledge` и задать вопрос по controlled-RAG заметкам, например «на какой неделе была самая длинная пробежка»;
...
GET /functions/v1/health -> {service, status:"ok", checks.database.reachable:true, checkedAt}
...
POST /functions/v1/ai-weekly-review с publishable/anon JWT -> 401 (реальный smoke 20 июня 2026)
...
Это веб-приложение — AI-планировщик личных и рабочих целей с AI-уточнением, AI-планом задач, дашбордом прогресса, еженедельным AI-ревью и controlled-RAG ответами по переданным личным заметкам.
